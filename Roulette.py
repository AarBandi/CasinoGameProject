...
def play_Roulette(money):
    ...
    #main run function
    def run_roulette(money_in):
        #global variables and initializing booleans
        global money_box, bet_amount
        #using a dict to store money to make sure that money cna be called in nested functions as well
        money_box = {"value": money_in}
        bet = True
        ...
        while bet:
            ...
            bet_amount = input(f'you have {money_box["value"]} how much would u like to bet (no decimals / cents): ')

            #checks if it can be converted to a int
            #by trying to convert bet_amount
            try:
                int(bet_amount)
                is_int = True

            #if there is an error bet_amount is not a int
            except ValueError:
                is_int = False

            #checks if bet_amount is a int or more than you can afford
            #then would ask you again based on our while loop
            if  is_int == False or int(bet_amount) > money_box["value"] :
                print('you cant bet this amount because its more then you have ')
                continue
            
            bet_amount = int(bet_amount)
            #if bet_amount is a int, and you have enough money then
            #we make bet false and to close the loop and take away your money you spent on the bet
            if is_int == True and bet_amount <= money_box["value"]  :
                bet = False
                money_box["value"] -= bet_amount
    ...
    def check_click(event):
        ...
        if bounds_checker(event.x, event.y):
            ...
            #turns bet amount into another original bet to find net gain
    ...
    your bet was {org_bet_amount} which got you a return of {bet_amount - org_bet_amount}''')
            ...
            play_again = input('would you like to play again (type y for yes and anything else for no)')

            #if they hit enter we run again carrying over the money
            if play_again == 'y':
                run_roulette(money_box["value"])
